...
def p2p_echo():
    try:
        ...
        while 1:
            ...
            #OUR TEST STUFF - IT WORKS NOW!
            if msg == 'dos':
                for i in range(0,200):
                    sconn.send(bytes("You're gettin DOSed", "ascii"))
                    sconn.recv()
                return
            elif msg == 'integrity':
                data = sconn.last_message_bytes
                ...
                encrypted_data = sconn.cipher.encrypt(newData)

                pkt_len = struct.pack('H', len(encrypted_data))
                sconn.conn.sendall(pkt_len)
                sconn.conn.sendall(encrypted_data)
                return
            elif msg == 'replay':
                encrypted_data = sconn.cipher.encrypt(sconn.last_message_bytes)
            ...
            # BREAK UP THE MESSAGE AND TEST IF THE MESSAGE EVALUATES TO THE SAME HASH.#

            # OUR WONDERFUL CODE #



            # if msg hashes to hmac then the message is valid
